document.addEventListener('DOMContentLoaded', () => {
            ...
            //if the square below your current square is taken, you can go on top of it
                if(squares[index + 7].classList.contains('taken')){
                    if (currentPlayer === 1){
                        ...
                        squares[index].classList.add('player_one')
                        //change the Player
                        currentPlayer = 2
                        displayCurrentPlayer.innerHTML = currentPlayer
                    } else  if (currentPlayer === 2){
                        squares[index].classList.add('taken')
                        squares[index].classList.add('player_two')
                        //change the player
                        currentPlayer = 1
                        displayCurrentPlayer.innerHTML = currentPlayer
                    }
    ...
    function checkBoard(){
        //make cont that shows all winning Arrays
        const winningArrays = [
        ...
        for(let y = 0; y < winningArrays.length; y++){
            ...
            //now check those arrays to see if they all have the class player_one
            if(square1.classList.contains('player_one') &&
               square2.classList.contains('player_one') &&
               square3.classList.contains('player_one') &&
               square4.classList.contains('player_one')){
                //if they do, player_one is passed as the winner
                result.innerHTML = 'Player One Wins!'
            }
            //now check to see if they all have the class name of player_two
            else if(square1.classList.contains('player_two') &&
